...
module.exports = class extends Command {
    ...
    async run(msg, [name, topic = '']) {
        ...
        await database.query("SELECT * FROM issues", [], (err, rows, fields) => {
            ...
            templateChannel.clone({name:`issue-${rows[0].issueNum}-${name}`}).then(tChannel => tChannel.setTopic(topic).then(tChan => tChan.setParent("383719583124553729"))).catch(e => {
                ...
                if(e.message == "Invalid Form Body\nname: Text channel names must be alphanumeric with dashes or underscores.") {
                    ...
                    await database.query(`REPLACE INTO issues (issue, issueNum) VALUES (${rows[0].issue}, ${rows[0].issueNum+1})`, [], (err, rows, fields) => {
    ...
    async init() {
        ...
        await database.query("CREATE TABLE IF NOT EXISTS issues (issue VARCHAR(5) PRIMARY KEY, issueNum INT)", [], (err, rows, fields) => {
            if (err) console.log("Error on issue table creation:\n" + err)
        });
    }
