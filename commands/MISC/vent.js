...
module.exports = class extends Command {
    ...
    async run(msg, [vent]) {
        ...
        await database.query("INSERT INTO vents (userId, vent, date) VALUES (?, ?, ?)", [msg.author.id, vent, moment.utc().format("dddd, MMMM Do YYYY, h:mm:ss a")], (err, rows, fields) => {
            if (err) return console.log("Error on vent insert:\n" + err)
        })


        let id;
        await database.query(`SELECT * FROM anonIds WHERE userId="${msg.author.id}"`, [], async (err, rows, fields) => {
            ...
            if (rows.length >= 1) {
                ...
                await database.query("REPLACE INTO anonIds (userId) VALUES (?)", [msg.author.id], async (err, rows, fields) => {
                    if (err) return console.log("Error on anonId insert:\n" + err)
                    await database.query(`SELECT * FROM anonIds WHERE userId="${msg.author.id}"`, [], (err, rows, fields) => {
                        if (err) return console.log("Error on anonId select:\n" + err)
                        if (rows.length >= 1) {
                            id = rows[0].id
                        }
    ...
    async init() {
        ...
        await database.query("CREATE TABLE IF NOT EXISTS vents (userId TEXT, vent TEXT, date TEXT)", [], (err, rows, fields) => {
            if (err) console.log("Error on vent table creation:\n" + err)
        });
        await database.query("CREATE TABLE IF NOT EXISTS anonIds (userId VARCHAR(19) PRIMARY KEY, id INT UNSIGNED NOT NULL AUTO_INCREMENT UNIQUE KEY)", [], (err, rows, fields) => {
