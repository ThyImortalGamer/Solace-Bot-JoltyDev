...
module.exports = class extends Command {
    ...
    async run(msg, [member]) {
        ...
        if(!member) {
            if (!msg.member.roles.has(fullmember)) return msg.reply('You have not opted in yet.')
            ...
            rows.forEach(row => {
                if (!rows) {
                    return msg.reply(`You haven't got a DB entry. If you haven't already, please do !optout, then !optin and you will get one. If you think this is a mistake, please contact one of our Keepers of the Code.`)
                        ...
                        console.log(`Level: ${row.level} | XP: ${row.points}/${maxxp} | Points: ${row.credits} | Colour: ${row.Colour} | Booster: ${booster} `)
                    }
                })
